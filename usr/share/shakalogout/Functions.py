...
working_dir = ''.join([str(Path(__file__).parents[2]), "/share/shakalogout-themes/"])
# config = "/etc/shakalogout.conf"
if os.path.isfile(home + "/.config/shakalogout/shakalogout.conf"):
    config = home + "/.config/shakalogout/shakalogout.conf"
else:
    config = ''.join([str(Path(__file__).parents[3]), "/etc/shakalogout.conf"])
root_config = ''.join([str(Path(__file__).parents[3]), "/etc/shakalogout.conf"])
...
def cache_bl(self, GLib, Gtk):
    if os.path.isfile("/usr/bin/betterlockscreen"):
        ...
        os.unlink("/tmp/shakalogout.lock")
...
def get_config(self, Gdk, Gtk, config):
    try:
        ...
        print(e)
        os.unlink(home + "/.config/shakalogout/shakalogout.conf")
        if not os.path.isfile(home + "/.config/shakalogout/shakalogout.conf"):
            shutil.copy(root_config, home + "/.config/shakalogout/shakalogout.conf")
