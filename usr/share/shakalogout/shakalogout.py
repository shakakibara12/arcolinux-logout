...
class TransparentWindow(Gtk.Window):
    ...
    wallpaper = "/home/shaka/Pictures/Wallpaper/wallpaper"
    d_buttons = ['cancel',
                 'shutdown',
                 'restart',
                 'suspend',
    ...
    def __init__(self):
        ...
        if not fn.os.path.isdir(fn.home + "/.config/shakalogout"):
            fn.os.mkdir(fn.home + "/.config/shakalogout")

        if not fn.os.path.isfile(fn.home + "/.config/shakalogout/shakalogout.conf"):
            shutil.copy(fn.root_config, fn.home + "/.config/shakalogout/shakalogout.conf")
        ...
        if not fn.os.path.isfile("/tmp/shakalogout.lock"):
            with open("/tmp/shakalogout.lock", "w") as f:
                f.write("")
    ...
    def on_save_clicked(self, widget):
        ...
        try:
            with open(fn.home + "/.config/shakalogout/shakalogout.conf", "r") as f:
                lines = f.readlines()
                f.close()
            ...
            with open(fn.home + "/.config/shakalogout/shakalogout.conf", "w") as f:
                f.writelines(lines)
                f.close()
            self.popover.popdown()
        except Exception as e:
            fn.os.unlink(fn.home + "/.config/shakalogout/shakalogout.conf")
            if not fn.os.path.isfile(fn.home + "/.config/shakalogout/shakalogout.conf"):
                shutil.copy(fn.root_config, fn.home + "/.config/shakalogout/shakalogout.conf")
            with open(fn.home + "/.config/shakalogout/shakalogout.conf", "r") as f:
                lines = f.readlines()
    ...
    def click_button(self, widget, data=None):
        ...
        if (data == self.binds.get('logout')):
            ...
            fn.os.unlink("/tmp/shakalogout.lock")
            fn.os.unlink("/tmp/shakalogout.pid")
...
if __name__ == "__main__":
    ...
    if not fn.os.path.isfile("/tmp/shakalogout.lock"):
        with open("/tmp/shakalogout.pid", "w") as f:
